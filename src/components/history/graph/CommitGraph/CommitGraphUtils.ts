...
function topologicalOrderCommits(
  ...
  function dfs(commit: CommitGraphCommitNode) {
...
export function computePosition(commits: CommitGraphCommitNode[]) {
  const commitsMap = new Map<string, CommitGraphCommitNode>(
